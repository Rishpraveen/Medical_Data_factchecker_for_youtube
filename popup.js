...
function setProcessing(processing, progress = 0, statusText = 'Processing...') {
    isProcessing = processing;
    
    if (processing) {
        processingIndicator.style.display = 'block';
        processingIndicator.classList.add('shake-fix');
        
        // Update processing text and progress
        const processingTextElement = processingIndicator.querySelector('.processing-text');
        if (processingTextElement) {
            processingTextElement.textContent = statusText;
        }
        
        // Add or update progress percentage
        let progressElement = processingIndicator.querySelector('.progress-percentage');
        if (!progressElement) {
            progressElement = document.createElement('div');
            progressElement.className = 'progress-percentage';
            processingIndicator.appendChild(progressElement);
        }
        progressElement.textContent = `${Math.round(progress)}% complete`;
        
        // Add progress bar background
        let progressBarBg = processingIndicator.querySelector('.progress-bar-bg');
        if (!progressBarBg) {
            progressBarBg = document.createElement('div');
            progressBarBg.className = 'progress-bar-bg';
            progressBarBg.innerHTML = '<div class="progress-bar-fill"></div>';
            processingIndicator.appendChild(progressBarBg);
        }
        
        const progressFill = progressBarBg.querySelector('.progress-bar-fill');
        if (progressFill) {
            progressFill.style.width = `${progress}%`;
        }
    } else {
        processingIndicator.style.display = 'none';
        processingIndicator.classList.remove('shake-fix');
    }
    
    // Disable/enable buttons
    if (transcriptBtn) transcriptBtn.disabled = processing;
    if (commentsBtn) commentsBtn.disabled = processing;
    if (ragBtn) ragBtn.disabled = processing;
...
transcriptBtn.addEventListener('click', () => {
    ...
    setProcessing(true, 10, 'Initializing transcript extraction...');
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (tabs[0]?.id && tabs[0]?.url && isYouTubeVideoUrl(tabs[0].url)) {
            ...
            setProcessing(true, 25, 'Connecting to YouTube...');
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === "updateStatus") {
        ...
        if (request.isProcessing !== undefined) {
            ...
            const statusText = request.statusText || request.message || 'Processing...';
            setProcessing(request.isProcessing, progress, statusText);
        }
        ...
        const progress = request.progress || 0;
        const statusText = request.statusText || `Processing... ${Math.round(progress)}%`;
        setProcessing(true, progress, statusText);
        updateProgress(request.type, progress);
        ...
        if (request.data) {
            ...
            if (request.data.type === 'multi-language') {
                ...
                updateStatus('✅ Transcript processed successfully.');
            }
